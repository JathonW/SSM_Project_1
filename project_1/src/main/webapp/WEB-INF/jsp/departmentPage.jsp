...
<body>
<div class="jathow_dept_container">
    <!-- 导航栏-->
    <%@ include file="./commom/head.jsp"%>


    <!-- 中间部分（左侧栏+表格内容） -->
    <div class="jathow_dept_body">
        <!-- 左侧栏 -->
            ...
            <div class="panel panel-success">
                ...
                <div class="panel-body">
                    ...
                    <nav aria-label="Page navigation" class="pull-right">
                        <ul class="pagination">
                            <li><a href="/jathow/dept/getDeptList?pageNo=1">首页</a></li>
                            ...
                            <c:forEach begin="1" end="${totalPages<5?totalPages:5}" step="1" var="itemPage">
                                <c:if test="${curPageNo == itemPage}">
                                    <li class="active"><a href="/jathow/dept/getDeptList?pageNo=${itemPage}">${itemPage}</a></li>
                                </c:if>
                                <c:if test="${curPageNo != itemPage}">
                                    <li><a href="/jathow/dept/getDeptList?pageNo=${itemPage}">${itemPage}</a></li>
                                </c:if>
                            </c:forEach>
                            ...
                            <li><a href="/jathow/dept/getDeptList?pageNo=${totalPages}">尾页</a></li>
                        </ul>
                    </nav>
                </div>
            </div><!-- /.panel panel-success -->
        </div><!-- /.dept_info -->
    </div><!-- /.jathow_dept_body -->
...
<script type="text/javascript">
    ...
    $(".prePage").click(function () {
         if (curPageNo > 1){
             ...
             $(this).attr("href", "/jathow/dept/getDeptList?pageNo="+pageNo);
         }
    });
    //下一页
    ...
    $(".dept_delete_btn").click(function () {
        ...
        if (confirm("确认删除【"+ delDeptName +"】的信息吗？")){
            $.ajax({
                url:"/jathow/dept/delDept/"+delDeptId,
                type:"DELETE",
                success:function (result) {
                    if (result.code == 100){
                        alert("删除成功！");
                        window.location.href = "/jathow/dept/getDeptList?pageNo="+curPageNo;
                    }else {
